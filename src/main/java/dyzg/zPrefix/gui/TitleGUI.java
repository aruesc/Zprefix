...
public class TitleGUI {
    ...
    public void openGUI(Player player, int page) {
        ...
        // 在打开GUI前清理无效的称号数据
        cleanupInvalidTitles(player, playerData);
    ...
    private ItemStack createRefreshButton() {
        ...
        List<String> lore = configManager.getConfigValue("gui.buttons.refresh.lore", 
            Arrays.asList("§7点击刷新界面", "§7重新加载称号数据"));
     ...
     */
    private void cleanupInvalidTitles(Player player, PlayerTitleData playerData) {
        int cleanedCount = titleManager.cleanupInvalidTitles(player);

        // 如果清理了数据，显示提示信息给玩家
        if (cleanedCount > 0) {
            player.sendMessage("§e§l[称号系统] §7检测到 §c" + cleanedCount + " §7个无效称号已被自动清理");
        }
    }
